...
module.exports = {
	...
	run: async (message, args, client, prefix) => {		
		...
		if (botchannel.id === channel.id) return message.reply(`${client.emoji.id} I am already connected in: \`${botchannel.name}\``);
		const e = await message.react('🎙️').catch(e => console.log(e))
		let VoiceConnection = await joinVoiceChannel({ channelId: channel.id ,guildId: channel.guild.id,adapterCreator: channel.guild.voiceAdapterCreator });
